...
void BattleHUD::DrawStatusBar(BattleMonster& mon, float x, float y, float width)
{
	...
	int color =	(mon.CurrentHP <= mon.data->HP / 4) ? GetColor(200, 80, 80)			// HPが1/4以下なら赤
			 : (mon.CurrentHP <= mon.data->HP / 2) ? GetColor(200, 200, 80)			// HPが1/2以下なら黄色
													: GetColor(80, 200, 80);		// HPが1/2以上なら緑
	DrawStatBar(x, barY, width, mon.CurrentHP, mon.data->HP, color);				// HPバーの描画
}
