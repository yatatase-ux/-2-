...
CpuDecisionResult CpuBrain::Decide(CpuEvalContext ctx, bool checkOscillation)
{
	...
	bool wantsToSwitch = (bestSwitchIndex >= 0 && bestSwitchScore > bestMoveScore);	// 交代した方が得なら交代する
	...
	if (wantsToSwitch && checkOscillation)
	{
		...
		candidateCtx.opponentPredictedToSwitch = ctx.opponentPredictedToSwitch; // 相手の交代予測はそのまま引き継ぐ
		candidateCtx.predictedOpponentDecision = ctx.predictedOpponentDecision; // 相手の行動予測もそのまま引き継ぐ
